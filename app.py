...
def build_trace_player_pts_per_team(type = 'bar', name = 'Default Table', title = 'Cumulative Squad Fantasy Points'):
    teamnames = [item.name for item in team_list()]
    team_player_points = [item.player_points for item in team_list()]
    trace1 = dict(x = teamnames, y = team_player_points, type = type, name = name)
    layout = go.Layout(
    title= title,
    xaxis=dict(
    title='Team Name',
...
def build_trace_money_team_comparison(type = 'bar', title = "AI's Smart Picks vs. The Common Folk's Picks"):
...
def money_team_composition_pie(title = 'Money Team Composition', type = 'pie', name = 'Money Team'):
...
def build_trace_all_players_points(title = 'Player Points vs Player Cost', type = 'scatter', mode = 'markers'):
    ...
    layout = dict(title = title, xaxis = dict(title = 'Player Cost(GPB millions)'), yaxis = dict(title = 'Total Fantasy Points'))
    return dict(data = [trace1], layout =layout)

def build_trace_top_vs_bottom_roi(type = 'bar'):
    ...
    return dict(data = [trace1, trace2, trace3])
...
app.layout = html.Div([
    dcc.Tabs(id="tabs", children=[
        ...
        dcc.Tab(label='Player Points Vs Cost', children=[
            html.Div([
                dcc.Graph(
                    id='example-graph',
